...
LLM은 중요한 결정도 사용자에게 묻지 않고 스스로 결정하고, 기존 구조에 미치는 영향이나 추후 확장 계획을 충분히 고려하지 않은 채 구현에 돌입하는 경향이 있습니다.
이 워크플로우는:
- **결정 통제** — 아키텍처/설계 결정은 Q&A를 통해 반드시 사용자가 직접 결정
- **신중한 설계** — 구현 전 기존 구조 영향과 확장 계획까지 검토
- **상태 추적** — plan 파일로 결정사항과 진행 상태 기록 (중단/재개/핸드오프)
